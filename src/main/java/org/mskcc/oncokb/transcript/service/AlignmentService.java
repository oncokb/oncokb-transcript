...
import org.mskcc.oncokb.transcript.domain.Alignment;
import org.mskcc.oncokb.transcript.domain.AlignmentResult;
import org.springframework.stereotype.Service;

/**
 * Author: https://github.com/SleekPanther/sequence-alignment
...
public class AlignmentService {
    ...
    public AlignmentService(
        int gapPenalty,
        int vowelVowelMismatchPenalty,
        int consonantConsonantMismatchPenalty,
        int vowelConsonantMismatchPenalty,
        int numberNumberMismatchPenalty
    ) {
    ...
    public AlignmentResult calcOptimalAlignment(String sequence1Original, String sequence2Original, boolean printResults) {
        ...
        if (printResults) {
            ...
            System.out.println(
                "\n" +
                minimumPenalty +
                "\t is the Minimum penalty for aligning \"" +
                sequence1Original +
                "\" with \"" +
                sequence2Original +
                "\""
    ...
    private String sanitizeSequence(String sequence) {
        return (
            " " +
            sequence //prepend a space @ the start. Allows for easier calls to mismatchPenalty() & array boundaries for size of memoTalbe to be "<" instead of "<="
                .toUpperCase()
                .replaceAll("[^A-Z0-9]", "")
        ); //remove all characters other than letters & digits (trims surrounding whitespace & removes spaces too). a-z matches the lowercase alphabet since previous line guarantees lowercase
    }
}
